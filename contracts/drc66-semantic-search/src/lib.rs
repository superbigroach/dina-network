...
impl SemanticSearchState {
    ...
    pub fn search(
        ...
        let mut eligible: Vec<&SearchProvider> = self
            .providers
            .values()
    ...
    pub fn submit_search_result(
        ...
        let req = self
            .search_requests
            .get_mut(&request_id)
            .expect("DRC66: request not found");
        assert!(
            req.providers_used.contains(&caller),
            "DRC66: not a selected provider"
        );
        ...
        assert!(
            !req.results.iter().any(|r| r.provider == caller),
            "DRC66: already submitted"
        );
    ...
    pub fn rate_result(
        ...
        let p = self
            .providers
            .get_mut(&key)
            .expect("DRC66: provider not found");
        p.total_ratings += 1;
        p.rating_sum += rating;
    ...
    pub fn best_providers(&self, top_n: usize) -> Vec<&SearchProvider> {
        let mut active: Vec<&SearchProvider> =
            self.providers.values().filter(|p| p.active).collect();
...
mod tests {
    ...
    fn test_dispatch_roundtrip() {
        ...
        let best = dispatch(
            &mut state,
            "best_providers",
            &serde_json::to_vec(&TopNArgs { top_n: 5 }).unwrap(),
            REQUESTER,
        );
