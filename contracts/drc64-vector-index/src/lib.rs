...
impl VectorIndexState {
    ...
    pub fn register_index(
        ...
        self.indexes.insert(id, VectorIndex {
            id,
            owner: caller,
            device_id,
            name,
    ...
    pub fn update_index(
        ...
        let idx = self.indexes.get_mut(&index_id).expect("DRC64: index not found");
    ...
    pub fn query_index(
        ...
        assert!(payment >= idx.price_per_query, "DRC64: insufficient payment");
        ...
        self.queries.insert(qid, VectorQuery {
            id: qid,
            requester: caller,
            index_id,
            query_hash,
    ...
    pub fn submit_result(
        ...
        let q = self.queries.get_mut(&query_id).expect("DRC64: query not found");
        let idx = self.indexes.get(&q.index_id).expect("DRC64: index not found");
        assert!(caller == idx.owner, "DRC64: only index owner can submit results");
...
struct RegisterIndexArgs {
    device_id: String, name: String, dimensions: u16, vector_count: u64,
    distance_metric: DistanceMetric, description: String, endpoint_hash: Vec<u8>,
    price_per_query: u64, timestamp: u64,
}
#[derive(Serialize, Deserialize, Debug)]
struct UpdateIndexArgs { index_id: IndexId, vector_count: u64, endpoint_hash: Vec<u8>, timestamp: u64 }
#[derive(Serialize, Deserialize, Debug)]
struct QueryIndexArgs { index_id: IndexId, query_hash: Vec<u8>, payment: u64, timestamp: u64 }
#[derive(Serialize, Deserialize, Debug)]
struct SubmitResultArgs { query_id: u64, result_hash: Vec<u8> }
#[derive(Serialize, Deserialize, Debug)]
struct IndexIdArgs { index_id: IndexId }
#[derive(Serialize, Deserialize, Debug)]
struct DimensionsArgs { dimensions: u16 }
#[derive(Serialize, Deserialize, Debug)]
struct MetricArgs { metric: DistanceMetric }
#[derive(Serialize, Deserialize, Debug)]
struct DeviceIdArgs { device_id: String }
...
pub fn dispatch(
    ...
    match method {
        ...
        "register_index" => {
            ...
            let id = s.register_index(caller, a.device_id, a.name, a.dimensions, a.vector_count,
                a.distance_metric, a.description, a.endpoint_hash, a.price_per_query, a.timestamp);
            serde_json::to_vec(&id).unwrap()
        }
        "update_index" => {
            let s = state.as_mut().expect("DRC64: not initialised");
            let a: UpdateIndexArgs = serde_json::from_slice(args).expect("DRC64: bad args");
            s.update_index(caller, a.index_id, a.vector_count, a.endpoint_hash, a.timestamp);
            serde_json::to_vec("ok").unwrap()
        }
        "query_index" => {
            let s = state.as_mut().expect("DRC64: not initialised");
...
mod tests {
    ...
    fn setup_with_index() -> (VectorIndexState, IndexId) {
        ...
        let id = s.register_index(
            PROVIDER, "seed-001".into(), "embeddings-384".into(),
            384, 50_000, DistanceMetric::Cosine,
            "General text embeddings".into(), vec![0xAB, 0xCD], 10, 1000,
        );
        (s, id)
    }
    ...
    fn test_search_by_dimensions() {
        let mut s = VectorIndexState::new(OWNER);
        s.register_index(PROVIDER, "s1".into(), "small".into(), 128, 1000,
            DistanceMetric::L2, "".into(), vec![], 5, 100);
        s.register_index(PROVIDER, "s2".into(), "large".into(), 768, 5000,
            DistanceMetric::Cosine, "".into(), vec![], 20, 200);
        s.register_index(PROVIDER, "s3".into(), "also-small".into(), 128, 2000,
            DistanceMetric::DotProduct, "".into(), vec![], 8, 300);
    ...
    fn test_indexes_by_device() {
        let mut s = VectorIndexState::new(OWNER);
        s.register_index(PROVIDER, "seed-A".into(), "idx1".into(), 256, 100,
            DistanceMetric::Cosine, "".into(), vec![], 5, 100);
        s.register_index(PROVIDER, "seed-A".into(), "idx2".into(), 512, 200,
            DistanceMetric::L2, "".into(), vec![], 10, 200);
        s.register_index(PROVIDER, "seed-B".into(), "idx3".into(), 256, 300,
            DistanceMetric::Cosine, "".into(), vec![], 15, 300);
    ...
    fn test_dispatch_roundtrip() {
        ...
        let reg = serde_json::to_vec(&RegisterIndexArgs {
            device_id: "dev-1".into(), name: "test-idx".into(), dimensions: 256,
            vector_count: 1000, distance_metric: DistanceMetric::Cosine,
            description: "test".into(), endpoint_hash: vec![0xFF], price_per_query: 5, timestamp: 100,
        }).unwrap();
