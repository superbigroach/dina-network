...
impl ClusterState {
    ...
    pub fn create_cluster(&mut self, caller: Address, price_per_compute_unit: u64) -> u64 {
    ...
    pub fn remove_device(&mut self, caller: Address, cluster_id: u64, device_id: &str) {
        let cluster = self
            .clusters
            .get_mut(&cluster_id)
            .expect("DRC72: cluster not found");
    ...
    pub fn assign_workload(
        ...
        let workload = self
            .workloads
            .get_mut(&workload_id)
            .expect("DRC72: workload not found");
        assert!(
            workload.status == WorkloadStatus::Queued,
            "DRC72: workload not queued"
        );
        assert!(
            total_compute >= workload.required_compute,
            "DRC72: insufficient compute"
        );
        assert!(
            total_memory >= workload.required_memory,
            "DRC72: insufficient memory"
        );
    ...
    pub fn cluster_stats(&self, cluster_id: u64) -> ClusterStats {
        let cluster = self
            ...
            .get(&cluster_id)
            .expect("DRC72: cluster not found");
        let available_compute: u64 = cluster
            .devices
            .iter()
            .filter(|d| d.available)
            .map(|d| d.compute_power)
            .sum();
        let available_memory: u64 = cluster
            .devices
            .iter()
            .filter(|d| d.available)
            .map(|d| d.memory)
    ...
    pub fn available_capacity(&self, cluster_id: u64) -> (u64, u64) {
        let cluster = self
            ...
            .get(&cluster_id)
            .expect("DRC72: cluster not found");
        let compute: u64 = cluster
            .devices
            .iter()
            .filter(|d| d.available)
            .map(|d| d.compute_power)
            .sum();
        let memory: u64 = cluster
            .devices
            .iter()
            .filter(|d| d.available)
            .map(|d| d.memory)
...
#[derive(Serialize, Deserialize, Debug)]
struct AssignWorkloadArgs {
    cluster_id: u64,
    workload_id: u64,
    device_ids: Vec<String>,
}
#[derive(Serialize, Deserialize, Debug)]
struct CompleteWorkloadArgs {
    cluster_id: u64,
    workload_id: u64,
    result_hash: Vec<u8>,
}
...
mod tests {
    ...
    fn test_submit_and_assign_workload() {
        ...
        assert_eq!(
            s.workloads.get(&wid).unwrap().status,
    ...
    fn test_dispatch_roundtrip() {
        ...
        let args = serde_json::to_vec(&CreateClusterArgs {
            price_per_compute_unit: 5,
        })
        .unwrap();
